...
func TestFindSkillsDir(t *testing.T) {
	...
	execSkills := filepath.Join(root, "bin", "skills")
	if err := os.MkdirAll(execSkills, 0o755); err != nil {
		t.Fatal(err)
	}
	got := findSkillsDir(filepath.Join(root, "bin"))
	if got != execSkills {
		t.Fatalf("findSkillsDir beside binary = %q, want %q", got, execSkills)
	}

	parentRoot := t.TempDir()
	parentSkills := filepath.Join(parentRoot, "skills")
	if err := os.MkdirAll(parentSkills, 0o755); err != nil {
		t.Fatal(err)
	}
	got = findSkillsDir(filepath.Join(parentRoot, "bin"))
	if got != parentSkills {
		t.Fatalf("findSkillsDir parent = %q, want %q", got, parentSkills)
	}

	cwdRoot := t.TempDir()
	cwdSkills := filepath.Join(cwdRoot, "skills")
	if err := os.MkdirAll(cwdSkills, 0o755); err != nil {
		t.Fatal(err)
	}
	execDir := filepath.Join(cwdRoot, "deep", "bin")
	if err := os.MkdirAll(execDir, 0o755); err != nil {
		t.Fatal(err)
	}
	t.Chdir(cwdRoot)
	if got := findSkillsDir(execDir); got != "skills" {
		t.Fatalf("findSkillsDir cwd fallback = %q, want skills", got)
	}

	emptyRoot := t.TempDir()
	t.Chdir(emptyRoot)
	if got := findSkillsDir(filepath.Join(emptyRoot, "bin")); got != "" {
		t.Fatalf("findSkillsDir missing = %q, want empty", got)
	}
}
...
func TestCopyFile(t *testing.T) {
	...
	if err := os.WriteFile(src, []byte("hello skill"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func setupSkillsWorkdir(t *testing.T) string {
	...
	if err := os.WriteFile(filepath.Join(skillsRoot, "SKILL.md"), []byte("# Jira CLI Skill"), 0o644); err != nil {
		t.Fatal(err)
	}
	nested := filepath.Join(workDir, "skills", "jira-cli", "reference")
	if err := os.MkdirAll(nested, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(nested, "issue.md"), []byte("ref"), 0o644); err != nil {
		t.Fatal(err)
	}
	return workDir
}

func TestRunInstallSkill(t *testing.T) {
	setupSkillsWorkdir(t)
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)

	t.Run("success", func(t *testing.T) {
		stdout, _ := runRootOKCleanDomain(t, "install-skill")
		if !containsAny(stdout, "Installed:", "Skill installed") {
			t.Fatalf("expected install success, got: %s", stdout)
		}
		dest := filepath.Join(home, ".openclaw", "skills", "jira-cli", "SKILL.md")
	...
	t.Run("update existing", func(t *testing.T) {
		stdout, _ := runRootOKCleanDomain(t, "install-skill")
		if !containsAny(stdout, "Updated:") {
			t.Fatalf("expected update message, got: %s", stdout)
		}
	})

	t.Run("json", func(t *testing.T) {
		stdout, _ := runRootOKCleanDomain(t, "--json", "install-skill")
		if !strings.Contains(stdout, `"installedFiles"`) || !strings.Contains(stdout, `"targetDir"`) {
			t.Fatalf("expected JSON install result, got: %s", stdout)
		}
	})

	t.Run("no skills dir", func(t *testing.T) {
		emptyDir := t.TempDir()
		t.Chdir(emptyDir)
		runRootExpectSilentClean(t, ExitBadArgs, "install-skill")
	})

	t.Run("empty skills dir", func(t *testing.T) {
		emptySkills := t.TempDir()
		t.Chdir(emptySkills)
		if err := os.MkdirAll(filepath.Join(emptySkills, "skills"), 0o755); err != nil {
			t.Fatal(err)
		}
		runRootExpectSilentClean(t, ExitBadArgs, "install-skill")
	})
	...
	t.Run("target mkdir error", func(t *testing.T) {
		setupSkillsWorkdir(t)
